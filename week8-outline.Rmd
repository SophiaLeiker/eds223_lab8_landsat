...
#Looking at the details of the landsat scene
landsat_20180612
#gives the number of layers, and min and max values for each of the layers 
```
...
```{r include=TRUE}
#can use the c function to stack all the rasters
...
#### read in study sites
```{r include=TRUE}
#reading in an sf_object that has 5 polygons for vegetation types
...
```{r include=TRUE}
#taking the mean of all the sites
...
```{r include=TRUE}
#pivot longer taking data into a long format (things that were columns are now rows)
#renaming the value as NDVI
#string subsetting to get the date out of it --> mutating and then pulling out the year as chatracters 2-5
#the date is the last two characters, this will get us columns from year month day
#then uniting these characters back to a string, and transition this back to a date using libridate
...
#### plot results
```{r include=TRUE}
ggplot(sites_clean, aes(x=date, y=NDVI, group = study_site, col = study_site)) + #group by study site and color by study site
    geom_line()
